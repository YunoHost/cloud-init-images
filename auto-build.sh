...
virt-customize -a yunohost-${suite_nb}.qcow2 --hostname yunohost --update --install 'curl' --upload './cloud.cfg:/etc/cloud/cloud.cfg' --run-command "curl https://install.yunohost.org/${suite} | bash -s -- -a -d testing" --run-command "rm -f /etc/resolv.conf" --run-command "echo 'nameserver 89.234.141.66' > /etc/resolv.conf"
